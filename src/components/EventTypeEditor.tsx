...
import {
  ...
  Phone, MapPin, Copy, Download, AlertTriangle, LayoutGrid, MessageCircle, CheckCircle2, ArrowUpRight
} from 'lucide-react';
...
export default function EventTypeEditor({ uid, initialData, onClose, onSaved }: Props) {
  ...
  // 3. Embed Code (Now handled directly in modal)
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', background: '#ffffff', color: '#0f172a', overflow: 'hidden', fontFamily: "'Geist', 'Geist Sans', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      {/* EMBED CODE MODAL */}
      {showEmbedModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.5)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100001 }} onClick={() => setShowEmbedModal(false)}>
          <div style={{ background: '#ffffff', borderRadius: '24px', width: '90%', maxWidth: '540px', padding: '32px', boxShadow: '0 24px 48px rgba(0,0,0,0.12)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button 
                onClick={() => {
                  const code = `<!-- LinksMeet inline widget begin -->\n<div class="linksmeet-inline-widget" data-url="${window.location.origin}/book/${uid || 'demo'}/${form.slug || '15min'}" data-primary-color="${eventColor}" data-auto-sync="false" style="min-width:320px;height:700px;"></div>\n<script type="text/javascript" src="${window.location.origin}/widget.js" async></script>\n<!-- LinksMeet inline widget end -->`;
              ...
              <button 
                onClick={() => {
                  const code = `<!-- LinksMeet popup widget begin -->\n<div class="linksmeet-popup-widget" data-url="${window.location.origin}/book/${uid || 'demo'}/${form.slug || '15min'}" data-text="Book a meeting" data-color="${eventColor}" data-text-color="#ffffff"></div>\n<script type="text/javascript" src="${window.location.origin}/widget.js" async></script>\n<!-- LinksMeet popup widget end -->`;
                  navigator.clipboard?.writeText(code).catch(() => {});
