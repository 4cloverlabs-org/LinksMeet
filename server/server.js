...
app.get('/api/inbox', async (req, res) => {
  try {
    ...
    if (userError || !userData || !userData.google_tokens) {
    ...
    if (error.code === 403 || error.code === 401) {
      return res.status(403).json({ error: "Insufficient permissions. Please reconnect your Google account." });
    }
  ...
  try {
    ...
    const getBody = (payload) => {
      ...
      if (payload.parts) {
        ...
        // Fallback to text/plain if no HTML part is found
        for (const part of payload.parts) {
          if (part.mimeType === 'text/plain') {
            return decodeBase64Url(part.body.data);
          }
        }
        // If multipart/alternative etc, recurse
        for (const part of payload.parts) {
    ...
    body = getBody(response.data.payload) || response.data.snippet;
